    ...
    //    select * from lookup_answers where lookup_field_id = 107
      //  select item_name from lookup_fields where lookup_id = 107
        ...
        if($gid || $gid2 || $search || $userType || $userStatus) {
          $view_details->title = ($gid ? "Members of Group: $group_name" . ($gid2 ? " / " . $group_name2 : "") : "Employees List");  
          ...
          function exportSelected(event) {
              ...
              xhr.open(\'POST\', \'/app/controllers/EmployeeExport.php\', true);
              ...
              xhr.onload = function () {
                  if (xhr.status === 200) {
                      ...
                      link.download = \'complete_employee_list_edge.csv\';
          ...
          $(document).ready(function() {
            $(".filterButton").on("click", function() {
              ...
              $("td").closest("tr").hide(); // Hide all rows by default
        
              $("td").each(function() {
                if ($(this).text().trim() === divisionName) {
                  $(this).closest("tr").show(); // Show rows with matching division
                }
              });
            });
          });
      ...
      <div class="row">
      <button class="filterButton" data-division="ATM - Traffic"">Traffic</button>
      <button class="filterButton" data-division="ASM - Security"">Security</button>
      <button class="filterButton" data-division="AFM - Facilities"">Facilities</button>
      <button class="filterButton" data-division="ACM - Civil"">Civil</button>
      <button class="filterButton" data-division="APM - Pest"">Pest</button>
      <button onclick="exportSelected(event)">Export Selected</button>
...
";
          ...
          $view_details->num_per_page = 1300;
          $str .= '
        <script>
