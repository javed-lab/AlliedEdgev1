...
class TrainingAlliedLessonsController extends Controller {
...
//       $this->f3->set('Job_List', $job_list);
        ...
        $this->f3->set('page_title', 'Lesson List');
        $this->f3->set('base_url', $this->f3->get('base_url') . "/AlliedTraining/Lessons/");
        echo $template->render('header.htm');
        echo $template->render('nav.htm');
        echo $view->render('trainings/lessons/index.php');
...
//                    . " WHERE $where_cond  ORDER BY $order_by $sort_order LIMIT $length OFFSET $start");
        ...
        if (!empty($record_list)) {
            ...
            foreach ($record_list as $key => $val) {
                ...
                $edit_link = $base_url . '/AlliedTraining/Lessons/Edit?course_id=' . $val['course_id'] . '&id=' . $val['id'];
...
//                    $edit = $delete = '';
    ...
    function manageLessons($type, $course_id = 0, $id = 0) {
        ...
        $trainingAlliedCourseList = $this->db->exec("SELECT id,title FROM  training_allied_courses WHERE training_allied_courses.id='$course_id' ORDER BY title ASC");

        $this->f3->set('trainingAlliedCourseList', $trainingAlliedCourseList);
        # print_r($trainingAlliedCourseList); die;
        if ($type != 'create') {
            $select_records = "SELECT *";
            $data_detail = $this->db->exec("$select_records FROM training_allied_lessons WHERE training_allied_lessons.id=$id");
...
//            prd($_FILES);
            ...
            if ($type == 'create' || !empty($data_detail[0])) {
                ...
                if ($type == 'create') {
                    ...
                    $insertQuery = "INSERT INTO `training_allied_lessons` (`course_id`, `title`, `description`, `document1`, `video1`,`duration`, `created_by`, `updated_by`)"
                    ...
                    $upQuery = "UPDATE training_allied_lessons SET course_id = '$course_id', title = '$title',description = '$description',document1 = '$document1',video1 = '$video1',duration='$duration',updated_by = '$updated_by'  WHERE id = '$id'";
                    #echo $upQuery; die; 
        ...
        if (isset($data_detail[0]['id'])) {
            $this->f3->set('page_title', 'Edit Lesson');
        } else {
            $this->f3->set('page_title', 'Add Lesson');
        }
        ...
        echo $template->render('header.htm');
        echo $template->render('nav.htm');
        echo $view->render('trainings/lessons/create.php');
        echo $template->render('footer.htm');
    }
...
//delete Lesson
    function deleteLesson() {
        ...
        if (!empty($_POST)) {
            ...
            $str = $this->db->exec("UPDATE training_allied_lessons SET status='2' WHERE id=" . $id);
        }
        echo $str;
        die;
    }
