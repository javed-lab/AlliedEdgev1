...
//        $this->f3->set('Job_List', $job_list);
        $this->f3->set('page_title', 'Toolbox Course List');
        $this->f3->set('base_url', $this->f3->get('base_url')."/AlliedToolbox/Courses/");
        ...
        $afterFrom = " FROM toolbox_allied_courses ta_courses "
        ...
        echo $view->render('toolbox/courses/index.php');
    ...
    function getListAjax() {
        ...
        $select_records = "SELECT ta_courses.`id`, ta_courses.`title`, ta_courses.`start_date`, ta_courses.`end_date`, ta_courses.`description`, ta_courses.`is_required`,ta_courses.`status`,ta_courses.`created_at`,division.`item_name` division_name,ta_frequency.`title` freq_title";
        $select_num_rows = "SELECT count(ta_courses.id) as total_records";
       ...
       // prd($record_list);
        ...
        if (!empty($record_list)) {
            ...
            foreach ($record_list as $key => $val) {
               ...
               $training_link = $base_url . "/AlliedToolbox/ToolboxUser?course_id=" . $val['id'];
               
               $lesson_link = $base_url . "/AlliedToolbox/Lessons?course_id=". $val['id'];
               
               
               
               $edit_link = $base_url . '/AlliedToolbox/Courses/Edit?course-id=' . $val['id'];
                $delete_link = "javaScript:delete_course(" . $val['id'] . ")";
                //$view_link = "javaScript:view_more_detail(" . $val['id'] . ")";
                $lesson ='<a title="Edit" class="green-a" href="' . $lesson_link . '"> Lesson </a> <br>';
                $trainingLink = '<a title="Edit" class="green-a" href="' . $training_link . '"> Users Assigned  </a>';
  ...
  /*
    ...
    function manageCourse($type, $id = 0) {
      ...
      if ($type != 'create') {
          ...
          $course_detail = $this->db->exec("$select_records FROM toolbox_allied_courses WHERE toolbox_allied_courses.id=$id");
      ...
      if (!empty($_POST)) {
        //print_r($_POST); die;
          ...
          if ($type == 'create' || !empty($course_detail[0])) {
              ...
              $is_required = 2; //(!empty($this->f3->get('POST.is_required')))?1:0;
              ...
              if (empty($description))
                ...
                $toolbox_start_date = date('Y-m-d', strtotime($start_date));

                $toolbox_end_date = date('Y-m-d', strtotime($end_date));
...
//                die;
                ...
                if(strtotime($toolbox_start_date) > strtotime($toolbox_end_date))
...
if ($type == 'create') {
  ...
  $insertQuery = "INSERT INTO `toolbox_allied_courses` (`division_id`, `allied_frequency_id`, `is_required`, `title`, `description`,`start_date`,`end_date`)"
                          . " VALUES('$division_id','$allied_frequency_id',$is_required,'$title','$description','$toolbox_start_date','$toolbox_end_date')";
               ...
               $upQuery = "UPDATE toolbox_allied_courses SET division_id = '$division_id', allied_frequency_id = '$allied_frequency_id',`is_required`= '$is_required',title = '$title',description = '$description',start_date = '$toolbox_start_date', end_date='$toolbox_end_date'  WHERE id = '$id'";
...
// echo $upQuery;
              ...
              if ($course_id) {
                  ...
                  if ($type == 'create') {
                      ...
                      $data = array(
                          ...
                          'course_id' => $id // Add course_id to the response
      ...
      if ($course_id) {
          $course_detail = $this->db->exec("$select_records FROM toolbox_allied_courses WHERE toolbox_allied_courses.id=$course_id");
      }
      ...
      if (isset($course_detail[0]['id'])) {
          $this->f3->set('page_title', 'Edit Toolbox Course');
      } else {
          $this->f3->set('page_title', 'Add Toolbox Course');
      }
...
//      $this->f3->set('supplier_ids', $supplier_ids);
      ...
      echo $template->render('header.htm');
      echo $template->render('nav.htm');
      echo $view->render('toolbox/courses/create.php');
      echo $template->render('footer.htm');
  }
  ...
  function deleteCourse() {
    ...
    if (!empty($_POST)) {
        ...
        $str = $this->db->exec("UPDATE toolbox_allied_courses SET status='2' WHERE id=" . $id);
    }
    echo $str;
    die;
}
