...
class Rostering1Controller extends Controller {
    ...
    function __construct($f3) {
        ...
        if (!$this->db->connected()) {
            // If not connected, then initialize the database
            $this->db_init();
        }
...
//            die;
        ...
        ");
            ...
            $division_str = $this->division_nav($division_id, 'Rostering/RosterOverview', 0, 0, 1);
      ...
      <div class="fl">';
            ...
            for ($i = $day_count; $i; $i--) {
                $str .= '<a data-uk-tooltip title="Show rosters from ' . $i . ' Day' . ($this->pluralise($i)) . ' ago.<br />' . date("l, d/M/Y", strtotime("-$i days")) . '" class="division_nav_item ' . ($days == -$i ? "division_nav_selected" : "") . '" href="RosterOverview?days=' . -$i . ($division_id ? "&division_id=$division_id" : "") . '">' . $i . '</a>';
            }

            $str .= " &lt;&lt;  ";
            $str .= '<a data-uk-tooltip title="Show Today\'s Rosters.<br />' . date("l, d/M/Y") . '" class="division_nav_item ' . (!$days ? "division_nav_selected" : "") . '" href="RosterOverview?a=1' . ($view_mode ? "&view_mode=$view_mode" : "") . ($date_by ? "&date_by=$date_by" : "") . ($division_id ? "&division_id=$division_id" : "") . '">Today</a>';
            $str .= " &gt;&gt; ";
            for ($i = 1; $i <= $day_count; $i++) {
                $str .= '<a data-uk-tooltip title="Show rosters ' . $i . ' Day' . ($this->pluralise($i)) . ' from now.<br />' . date("l, d/M/Y", strtotime("$i days")) . '" class="division_nav_item ' . ($days == $i ? "division_nav_selected" : "") . '" href="RosterOverview?days=' . $i . ($division_id ? "&division_id=$division_id" : "") . '">' . $i . '</a>';
            }


            $str .= '&nbsp;&nbsp;<a class="list_a" href="' . $this->f3->get('main_folder') . 'Rostering/RosterOverview?a=1' . ($division_id ? "&division_id=$division_id" : "") . ($days ? "&days=$days" : "") . ($date_by ? "&date_by=$date_by" : "") . ($view_mode ? '">Edit Mode' : '&view_mode=$view_mode">View Mode') . '</a></div>';
...
//                echo "<br>";
                ...
                if ($by_day_mode) {
                    ...
                    if ($old_site != $site) {
                        ...
                        $str .= ($old_site ? "</td>" : "") . "<td class=\"roster_column\" valign=\"top\">"
                                . "<div style=\"margin-bottom: 5px;\">"
                                . "<input type=\"checkbox\" id=\"assignBookingId$roster_idin\" name=\"assignBookingId$roster_idin\">"
                                . "<label for=\"assignBookingId$roster_idin\">Assign Booking ID</label>"
                                . "</div>"
                                . "<div class=\"roster_col_header\">"
                                . "<a class=\"roster_col_a\" href=\"" . $this->f3->get('main_folder') 
                                . "Rostering/EditRoster?rid=$roster_idin" 
...
//            die;
    ...
    function Rosters() {
        ...
        $str .= $this->division_nav($division_id, 'Rostering1/Rosters', 0, 0, 1, ($weeks ? "&weeks=$weeks" : ""));
