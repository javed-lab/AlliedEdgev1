<?php
class ToolboxAlliedQuestionsController extends Controller {
  protected $f3;
  protected $db;
  ...
  function __construct($f3) {
   ...
   /*
    ...
    function index() {
       ...
       $dataDetailQuery .= " FROM toolbox_allied_lessons ta_lessons "
        ...
        $afterFrom = " FROM  toolbox_allied_lesson_questions ta_questions "
                    ." LEFT JOIN toolbox_allied_lessons ta_lessons ON ta_lessons.id=ta_questions.lesson_id  ";              
       ...
       # prd($dataDetail);
        ...
        $this->f3->set('base_url', $this->f3->get('base_url')."/AlliedToolbox/Questions/");
        echo $template->render('header.htm');
        echo $template->render('nav.htm');
        echo $view->render('toolbox/questions/index.php');
       ...
       // prd($record_list);
        ...
        if (!empty($record_list)) {
            ...
            foreach ($record_list as $key => $val) {
               ...
               $question_link = $base_url . "/Alliedtoolbox_/Questions?lesson_id=". $val['id'];
                $edit_link = $base_url . '/Alliedtoolbox/Questions/Edit?lesson_id='.$val['lesson_id'].'&id=' . $val['id'];
...
//                    $edit = $delete = '';
  ...
  /*
     ...
     */
     ...
     function create() {
        if($_REQUEST['lesson_id'] == 0){
            $this->f3->reroute('/Alliedtoolbox_/Courses');
          $lesson_id = 0;
        }else{
 ...
 */
  ...
  function manageLessons($type,$lesson_id=0, $id = 0) {
    ...
    $trainingAlliedLessonsList = $this->db->exec("SELECT id,title FROM  toolbox_allied_lessons WHERE toolbox_allied_lessons.id='$lesson_id' ORDER BY title ASC"); 
          
    $this->f3->set('trainingAlliedLessonsList', $trainingAlliedLessonsList);
    //print_r($trainingAlliedLessonsList); die;
    if ($type != 'create') {
        $select_records = "SELECT *";
        $data_detail = $this->db->exec("$select_records FROM toolbox_allied_lesson_questions WHERE toolbox_allied_lesson_questions.id=$id");
...
//                $error_mesg .= 'Currect Option is required<br>';     
          ...
          if ($type == 'create') {
            ...
            $insertQuery = "INSERT INTO `toolbox_allied_lesson_questions` (`lesson_id`, `question`, `option_row`, `option1`, `option2`,`option3`,`option4`,`correct_option`, `created_by`, `updated_by`)"
            ...
            $question_id1 = $this->db->exec($insertQuery);
             ...
             $upQuery = "UPDATE toolbox_allied_lesson_questions SET lesson_id = '$lesson_id', question = '$question',option_row = '$option_row',option1 = '$option1',option2 = '$option2',option3='$option3',option4='$option4',correct_option='$correct_option',updated_by = '$updated_by'  WHERE id = '$id'";
             #echo $upQuery; die; 
   ...
   // print_r($data_detail); die;
    ...
    if (isset($data_detail[0]['id'])) {
        $this->f3->set('page_title', 'Edit Toolbox Question');
    } else {
        $this->f3->set('page_title', 'Add Toolbox Question');
    }
    ...
    echo $template->render('header.htm');
    echo $template->render('nav.htm');
    echo $view->render('toolbox/questions/create.php');
    echo $template->render('footer.htm');
}
...
function deleteQuestion() {
  ...
  if (!empty($_POST)) {
      ...
      $str = $this->db->exec("UPDATE toolbox_allied_lesson_questions SET status='2' WHERE id=" . $id);
  }
  echo $str;
  die;
}
