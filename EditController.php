...
class EditController extends Controller {
    ...
    function NoticeBoard() {
        ...
        order by notice_board.sort_order asc,updated_at desc
      ";
...
//        die;
        ...
        $this->list_obj->sql = "
    SELECT CONCAT('<input type=\"checkbox\" id=\"recid[]\" class=\"reccheck\" name=\"recid[]\" value =\"', main_pages2.id, '\">') as '<input type=\"checkbox\" id=\"pageid[]\" class=\"allreccheck\" onclick=\"allreccheck()\" name=\"pageid[]\">' ,main_pages2.id as `idin`, main_pages2.id as `ID`, main_pages2.parent_id as `PID`, main_pages_view.title as `Parent`, main_pages2.sort_order as `Sort Order`, main_pages2.title as `Title`, user_level.item_name as `User Level`, max_user_level.item_name as `Max User Level`, main_pages2.url as `URL`, page_types.item_name as `Page Type`,
    ...
    ';
        $assignPage = '<div class="fr"><a class="list_a" href="javascript:void(0)" onClick="assignPageWithRole()"> AssignPage </a></div> ';
        //$this->editor_obj->editor_template = '<table><tr><td valign="top">editor_form</td><td valign="top">editor_list</td></tr></table>';
        $this->editor_obj->editor_template = 'editor_form<hr />' . $assignPage . '</br>' . 'editor_list';
    ...
    function AssignPageRoleWise() {
        $pageIds = $_REQUEST['id'];
        $roleIds = $_REQUEST['roleId'];
...
//        $delete_access = "";
        ...
        foreach ($roleIds as $roleId) {
            ...
            foreach ($pageIds as $pageId) {
                ...
                $delete_access = " delete from page_access where page_id  = '" . $pageId . "' and group_id = '" . $roleId . "';";
                $this->dbi->query($delete_access);
                //insert ignore into job_ads_questions (job_ad_id, question_id, sort_order)
            ...
            $pageQuery = " select id from main_pages2 mp2 where mp2.parent_id = '" . $parentId . "' and page_type_id = '" . $sideId . "'";

            $this->dbi->query($str_xtra);
            // print_r($this->dbi->error);
            $str_xtra_delete = " delete from lookup_answers where lookup_field_id = " . $roleId . " and table_assoc = 'main_pages2'  and foreign_id in (" . $pageQuery . ")" . ($vals ? " and foreign_id not in($vals);" : "");
            //$this->dbi->query($delete_access);
            //  echo $delete_access;
//             die;
            //if($str_xtra != ""){
...
//    $filter_string = "filter_string";
        ...
        $this->list_obj->sql = "
    	select licence_types.id as idin,licence_types.item_name as `Title`,      
      'Edit' as `*` from licence_types
      $filter_string
    	order by licence_types.id
      ";
        //return $this->list_obj->sql;
        $list_top = "select 0 as id, '--- Select ---' as title union all";
        $this->editor_obj->table = "licence_types";
        $style_small = 'style="width: 100px;"';
        $style = 'style="width: 190px;"';
        $style_large = 'style="width: 385px; height: 60px;"';
        
         $form_obj = $this->editor_obj;
            $formAttributeArr[] = array("txtTitle","Title", "item_name", "", $style_med, "c", "");            
...
//        }
        ...
        $this->editor_obj->editor_template = '
                ...
                editor_list
    ';
        //$this->editor_obj->xtra_id_name = "id";
        //$this->editor_obj->xtra_id = $lookup_id;
        $str .= $this->editor_obj->draw_data_editor($this->list_obj);
        $str .= '<input type="hidden" name="lookup_id" value="' . $lookup_id . '"><input type="hidden" name="show_max" value="' . $show_max . '">';
...
//    $filter_string = "filter_string";
        $filter_string = "";
        $this->list_obj->category_name = $category_name;

        $this->list_obj->sql = "
    	select jobs_category.id as idin,jobs_category.category_name as `Category Name`,      
      'Edit' as `*` from jobs_category
      $filter_string
    	order by jobs_category.category_name
      ";
        //return $this->list_obj->sql;
        $list_top = "select 0 as id, '--- Select ---' as category_name union all";
        $this->editor_obj->table = "jobs_category";
        $style_small = 'style="width: 100px;"';
        $style = 'style="width: 190px;"';
        $style_large = 'style="width: 385px; height: 60px;"';
        
         $form_obj = $this->editor_obj;
            $formAttributeArr[] = array("txtCategoryName","Category Name", "category_name", "", $style_large, "c", "");            
        ...
        $title = "Job Category ";
        $this->editor_obj->form_template = '
              <div class="fl"><nobr>ttxtCategoryName</nobr><br />txtCategoryName</div>             
              <div class="cl"></div>
              ' . $this->editor_obj->button_list();
        $this->editor_obj->editor_template = '
                <div class="form-wrapper" style="width: 100%;">
...
//        $locationList = $this->getLocationOfJobs(); 
//        pr($locationList);
//        $clientOfSupplierList = $this->getSupplierOfJobs(1);
//         pr($clientOfSupplierList);
//        $clientOfList = $this->getClientsOfJobs();
//        prd($clientOfList);
        
        
        
        $this->assignPageToRoleWise();
      ...
      ";
            ...
            $userType = $this->getUserType($edit_id);
             $this->createDefaultSiteFolder($edit_id);
           

            //prd($uid);
            if ($uid) {
                $str .= "<div class=\"fr\"><a class=\"list_a normal_button\" href=\"" . $this->f3->get('main_folder') . "UserCard?uid=$uid\">Open Card</a></div>";
            }
            if ($userType == 105 || $userType == 107) {
                $str .= "<div class=\"fr\"><a class=\"list_a normal_button \" href=\"" . $this->f3->get('main_folder') . "AssignSiteManager?uid=$uid\"> Assign State-Sites</a></div>";
            }
            
            
            if($edit_id){
                $str .= '<div class="fr"><a class="list_a normal_button" href="javascript:void(0)" onClick="deleteUser('.$edit_id.')"> Delete </a></div>';
                    //  $deletejs  = "'Delete' as `!`"; 
            }
      ...
      ';
        ...
        SELECT users.id as `idin`, CONCAT(if(employee_id IS NOT NULL && employee_id != '', CONCAT(employee_id, '<br />'), ''), if(client_id  IS NOT NULL && client_id != '', CONCAT(client_id, '<br />'), ''), if(supplier_id IS NOT NULL && supplier_id != '', CONCAT(supplier_id, '<br />'), '')) as `User IDs`, users.name as `First Name`, users.surname as `Surname`, users.email as `Email`, users.email2 as `Email 2`, users.phone as `Phone`,
               users.company as `Company`, user_level.item_name as `User Level`, users.commencement_date as `Start Date`, 
...
//        }
     ';
            ...
            if ($_SESSION['user_id'] && $_SESSION['user_id'] != 1 && $edit_id == 1) {
            ...
            $userRoleName = $this->userRoleNameByUid($edit_id);
...
//            /* section15 */
            ...
            $formAttributeArr[] = array("calTerminationDate", "Termination Date", "termination_date", "", $style, "n", " ");
            $formAttributeArr[] = array("txtTerminationReason", "Termination Reason", "termination_reason", "", $style, "");

//
//
//            /* section16 */
            $formAttributeArr[] = array("chlDivisions", "Select Allied Division:", "id", $division_sql, $style, "u", "user_divisions");
...
//            /* section11 */
            ...
            $formAttributeArr[] = array("txtSuperName", "Superannuation Name", "super_name", "", $style_med, "n", "");
            $formAttributeArr[] = array("txtSuperNumber", "Superannuation Number", "super_number", "", $style_med, "n", "");
...
//            /* section12 */
            ...
            /* for gps manage */
            $formAttributeArr[] = array("chkGpsSignOnOff", "Gps Sign On Off", "site_gps_on_off", "", "", "n", "", "");
            $formAttributeArr[] = array("txtGpsRadius", "Gps Radius(Meter)", "site_gps_radius", "", "", "n", "", "");
...
//      $formAttributeArr[] = array("cmbVisaType", "Visa Type", "visa_type", $this->get_cmb_lookup('visa_type'), $style, "n", ""); 
            $resultV = $form_obj->formAttributesVerticle($formAttributeArr);           
            if($edit_id && !empty($_REQUEST['supplier_category'])){    
              //  prd($_REQUEST['supplier_category']);
                        //die($_REQUEST['supplier_category']);
                            $this->updateSupplierCategory($edit_id,$_REQUEST['supplier_category']);                
                        }
            
                        
            if($edit_id){             
            $supplierSelectedCategory =  $this->supplierCategorySelected($edit_id);     
            }else{
                $supplierSelectedCategory = array();
            }
           ...
           $selectCategory .= '<option value="">--- Select ---</option>';
            foreach($supplierCategory as $supvalue){   
                ...
                if(in_array($supvalue['id'],$supplierSelectedCategory)){
                    $selectedCategory = "selected";
                }else{
                    $selectedCategory = "";
                }
                
                $selectCategory .= '<option value="'.$supvalue['id'].'" '.$selectedCategory.'>'.$supvalue['category_name'].'</option>';
            }            
        $selectCategory .= '</select>';   
            ...
            $this->editor_obj->button_attributes = array(
                ...
                array("js_function_save", "js_function_provider_check", "js_function_delete", "js_function_edit", "js_function_filter")
            );
      ...
      <hr />
      <!--  section 1 -->';
            if ($edit_id == 3 || $edit_id == 5) {
                $form_obj->form_template .= '<div class="fl ">tselUserLevel<br />selUserLevel</div>';
            } else {
                $form_obj->form_template .= '<div class="fl user3">tselUserLevel<br />selUserLevel</div>';
            }
            
            /* for civil Manger */
            
        $userDetail = $this->userDetail($edit_id);   
       ...
       $civilManagerList = $this->divisionManagerList(2103);
        ...
        $selectCivilManager1 = '<select class="manager_id" id="civil_manager_id" name="civil_manager_id" manager_type="Civil" manager_num="" style="width: 150px !important;">';
           $selectCivilManager1 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($civilManagerList as $civilManager){  
                $selected = "";
                if(isset($_REQUEST['civil_manager_id']))
                {
                   //prd($_REQUEST);
                ...
                }else if($civilManager['id'] == $userDetail['civil_manager_id']){
                    $selected = "selected";
                }
        ...
        $selectCivilManager1 .= '</select>';  
         ...
         $selectCivilManager2 = '<select class="manager_id" id="civil_manager_id2" name="civil_manager_id2" manager_type="Civil" manager_num="2" style="width:150px !important;">';
           $selectCivilManager2 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($civilManagerList as $civilManager){    
                  $selected = "";
                if(isset($_REQUEST['civil_manager_id2']))                
                {
                    if($civilManager['id'] == $_REQUEST['civil_manager_id2']){
                  
                        $selected = "selected";
                    }                                  
                }else if($civilManager['id'] == $userDetail['civil_manager_id2']){
                    $selected = "selected";
                }
        ...
        $selectCivilManager2 .= '</select>'; 
         ...
         $selectCivilManager3 = '<select class="manager_id" id="civil_manager_id3" name="civil_manager_id3" manager_type="Civil" manager_num="3" style="width: 150px !important;">';
           $selectCivilManager3 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($civilManagerList as $civilManager){ 
                 $selected = "";
                 if(isset($_REQUEST['civil_manager_id3'])) 
                {
                ...
                }else if($civilManager['id'] == $userDetail['civil_manager_id3']){
                    $selected = "selected";
                }
        ...
        $selectFacilitiesManager1 = '<select class="manager_id" id="facilities_manager_id" name="facilities_manager_id" manager_type="Facilities" manager_num="" style="width: 150px !important;">';
           $selectFacilitiesManager1 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($facilitiesManagerList as $facilitiesManagerManager){    
                  $selected = "";
                if(isset($_REQUEST['facilities_manager_id']))                
                {
                ...
                else if($facilitiesManagerManager['id'] == $userDetail['facilities_manager_id'])
                {
                    $selected = "selected";
                }
        ...
        $selectFacilitiesManager2 = '<select class="manager_id" id="facilities_manager_id" name="facilities_manager_id2" manager_type="Facilities" manager_num="2" style="width: 150px !important;">';
           $selectFacilitiesManager2 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($facilitiesManagerList as $facilitiesManagerManager){             
                  $selected = "";
                if(isset($_REQUEST['facilities_manager_id2']))                
                {
                ...
                else if($facilitiesManagerManager['id'] == $userDetail['facilities_manager_id2'])
                {
                    $selected = "selected";
                }
        ...
        $selectFacilitiesManager3 = '<select class="manager_id" id="facilities_manager_id3" name="facilities_manager_id3" manager_type="Facilities" manager_num="3" style="width: 150px !important;">';
           $selectFacilitiesManager3 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($facilitiesManagerList as $facilitiesManagerManager){   
                $selected = "";
                if(isset($_REQUEST['facilities_manager_id3']))
                {
                ...
                else if($facilitiesManagerManager['id'] == $userDetail['facilities_manager_id3'])
                {
                    $selected = "selected";
                }                
        ...
        $selectPestManager1 = '<select class="manager_id" id="pest_manager_id" name="pest_manager_id" manager_type="Pest" manager_num="" style="width: 150px !important;">';
           $selectPestManager1 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($pestManagerList as $pestManagerManager){  
                 ...
                 $selected = "";
                if(isset($_REQUEST['pest_manager_id']))                
                {
                ...
                else if($pestManagerManager['id'] == $userDetail['pest_manager_id'])
                {
                    $selected = "selected";
                }
        ...
        $selectPestManager1 .= '</select>'; 
          ...
          $selectPestManager2 = '<select class="manager_id" id="pest_manager_id2" name="pest_manager_id2" manager_type="Pest" manager_num="2" style="width: 150px !important;">';
           $selectPestManager2 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($pestManagerList as $pestManagerManager){             
                 $selected = "";
                if(isset($_REQUEST['pest_manager_id2']))  
                {
                ...
                else if($pestManagerManager['id'] == $userDetail['pest_manager_id2'])
                {
                    $selected = "selected";
                }
        ...
        $selectPestManager2 .= '</select>'; 
          ...
          $selectPestManager3 = '<select class="manager_id" id="pest_manager_id3"  name="pest_manager_id3" manager_type="Pest" manager_num="3" style="width: 150px !important;">';
           $selectPestManager3 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($pestManagerList as $pestManagerManager){ 
                 $selected = "";
                if(isset($_REQUEST['pest_manager_id3']))                
                {
                ...
                else if($pestManagerManager['id'] == $userDetail['pest_manager_id3'])
                {
                    $selected = "selected";
                }
        ...
        $selectSecurityManager1 = '<select class="manager_id" id="security_manager_id" name="security_manager_id" manager_type="Security" manager_num="" style="width: 150px !important;">';
           $selectSecurityManager1 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($securityManagerList as $securityManager){  
                 ...
                 $selected = "";
               if(isset($_REQUEST['security_manager_id']))                
                {
                ...
                else if($securityManager['id'] == $userDetail['security_manager_id'])
                {
                    $selected = "selected";
                }
         ...
         $selectSecurityManager2 = '<select class="manager_id" id="security_manager_id2" name="security_manager_id2" manager_type="Security" manager_num="2" style="width: 150px !important;">';
           $selectSecurityManager2 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($securityManagerList as $securityManager){  
                 ...
                 $selected = "";
                 if(isset($_REQUEST['security_manager_id2']))                
                {
                ...
                else if($securityManager['id'] == $userDetail['security_manager_id2'])
                {
                    $selected = "selected";
                }
         ...
         $selectSecurityManager3 = '<select class="manager_id" id="security_manager_id3" name="security_manager_id3" manager_type="Security" manager_num="3" style="width: 150px !important;">';
           $selectSecurityManager3 .= '<option value="" full_name="">--- Select ---</option>';
             foreach($securityManagerList as $securityManager){  
                 ...
                 $selected = "";
                if(isset($_REQUEST['security_manager_id3']))                
                {
                ...
                else if($securityManager['id'] == $userDetail['security_manager_id3'])
                {
                    $selected = "selected";
                }
        ...
        $selectTrafficManager1 = '<select class="manager_id" id="traffic_manager_id" name="traffic_manager_id" manager_type="Traffic" manager_num="" style="width: 150px !important;">';
           $selectTrafficManager1 .= '<option value="" full_name="">--- Select ---</option>';
            foreach($trafficManagerList as $trafficManager){  
                  $selected = "";
                if(isset($_REQUEST['traffic_manager_id']))                
                {
                ...
                else if($trafficManager['id'] == $userDetail['traffic_manager_id'])
                {
                    $selected = "selected";
                }
        ...
        $selectTrafficManager2 = '<select class="manager_id" id="traffic_manager_id2" name="traffic_manager_id2" manager_type="Traffic" manager_num="2" style="width: 150px !important;">';
           ...
           foreach($trafficManagerList as $trafficManager){  
                  $selected = "";
                if(isset($_REQUEST['traffic_manager_id2']))                
                {
                ...
                else if($trafficManager['id'] == $userDetail['traffic_manager_id2'])
                {
                    $selected = "selected";
                }
        ...
        $selectTrafficManager3 = '<select class="manager_id" id="traffic_manager_id3" name="traffic_manager_id3" manager_type="Traffic" manager_num="3" style="width: 150px !important;">';
           ...
           foreach($trafficManagerList as $trafficManager){  
                  $selected = "";
                if(isset($_REQUEST['traffic_manager_id3']))                
                {
                ...
                else if($trafficManager['id'] == $userDetail['traffic_manager_id3'])
                {
                    $selected = "selected";
                }
...
<div id="xtras">           
       <input type="file" style="padding: 0; margin: 0;" class="upload" name="fileToUpload" id="fileToUpload" onchange="loadImageFile()">
       <canvas id="myCanvas" style="max-width: 100px; height: 0px;"></canvas>
...
<!-- section 4a -->
<div class="comdiv user3div">
        <div class="cl"></div><br /><b>Employment Termination Details </b>  <div class="cl"></div> 
        <div class="fl section3  user3"><nobr>tcalTerminationDate</nobr><br />calTerminationDate</div>
      <div class="fl section3  user3"><nobr>ttxtTerminationReason</nobr><br />txtTerminationReason</div>
...
<div class="comdiv user2">
        <div class="cl"></div><br />    
        <b> GPS Settings </b></br>
        <div class="fl section2 user2" ><nobr>tchkGpsSignOnOff<nobr></br> chkGpsSignOnOff </div>
        <div class="fl section2  user2" style="margin-left:100px!important"><nobr>ttxtGpsRadius</nobr><br />txtGpsRadius</div>
        <div id="map" class="mapShow" style="width: 400px; height: 300px; margin-left:100px!important; padding-left:20px!important "></div>
        <div class="cl"></div><br />
<style>
.mapShow {
    ...
    <div class="comdiv user2">
      ...
      <div class="cl"></div><br />   <hr><div class="cl"></div><br />
</div>
...
<!--<div class="fl user3" style="display:none">tchlUserGroups<br />chlUserGroups</div>-->
 ...
 <div class="fl">Confirm Password<br /><input name="pw2" id="pw2" type="password" style="width: 190px;" autocomplete="new-password"></div>
      ...
      function initMap() {
          ...
          var gpsRadius = parseFloat(document.getElementById("txtGpsRadius").value);
          ...
          var circle = new google.maps.Circle({
              ...
              map: map,
              center: {lat: latitude, lng: longitude},
              radius: gpsRadius // Use the radius directly (in meters)
          });
      }
      ...
      if (typeof google === "object" && typeof google.maps === "object") {
          ...
          // Google Maps API is not yet loaded, wait for it to load and then initialize the map
...
//           if($("#hdn_user_types").val() == 107 && middleName.trim() == "")
//           {
//                 err++;
//                      error_msg += err + ". Please enter middle Name.\\n";
//           }
           
            if($("#hdn_user_types").val() == 107 && lastName.trim() == "")
            {
                  err++;
                       error_msg += err + ". Please enter surname.\\n";
            }
            
            if($("#hdn_user_types").val() == 107 && caldob.trim() == "")
            {
                  err++;
                       error_msg += err + ". Please select date of birth.\\n";
            }
            
            if($("#hdn_user_types").val() == 107 && txtphone2.trim() == "")
            {
                  err++;
            ...
            if($("#hdn_user_types").val() == 107 && divisionSelected == 0){
                    err++;
                      error_msg += err + ". Please select division.\\n";
            }

            if($("#selUserType").val() == 107 && document.getElementById("txtEmail").value == "") {
                    err++;
     ...
     ';
          ...
          if(document.getElementById("hdn_user_types").value == "105" && document.getElementById("supplier_category").value == "" ){
        err++;
...
//      '.$this->editor_obj->button_list();
            $usernameShow = "";
            if ($userType == 107) {
                //$usernameShow = 'Username ' . $getUserName."  UserRole: ".$userRoleName;
                $usernameShow = 'Username ' . $getUserName;
            }
            $this->editor_obj->editor_template = '
...
//            
                if ($save_id) {  
                    ...
                    if($save_id && !empty($_REQUEST['supplier_category'])){     
                        //die($_REQUEST['supplier_category']);
                            $this->updateSupplierCategory($save_id,$_REQUEST['supplier_category']);                
                        }
                      //  prd($_REQUEST);
//                        if($save_id && $_REQUEST['selUserType'] == "2436"){  
//                            die("hello test");
                            $sql = "update users set "
                                    . " civil_manager_id = '".$_REQUEST['civil_manager_id']."'"
                                    . " ,civil_manager_id2 = '".$_REQUEST['civil_manager_id2']."'"
                                     . " ,civil_manager_id3 = '".$_REQUEST['civil_manager_id3']."'"
                                    . " ,facilities_manager_id = '".$_REQUEST['facilities_manager_id']."'"
                                    . " ,facilities_manager_id2 = '".$_REQUEST['facilities_manager_id2']."'"
                                     . " ,facilities_manager_id3 = '".$_REQUEST['facilities_manager_id3']."'"
                                    . " ,pest_manager_id = '".$_REQUEST['pest_manager_id']."'"
                                    . " ,pest_manager_id2 = '".$_REQUEST['pest_manager_id2']."'"
                                     . " ,pest_manager_id3 = '".$_REQUEST['pest_manager_id3']."'"
                                    . " ,security_manager_id = '".$_REQUEST['security_manager_id']."'"
                                    . " ,security_manager_id2 = '".$_REQUEST['security_manager_id2']."'"
                                     . " ,security_manager_id3 = '".$_REQUEST['security_manager_id3']."'"
                                    . " ,traffic_manager_id = '".$_REQUEST['traffic_manager_id']."'"
                                    . " ,traffic_manager_id2 = '".$_REQUEST['traffic_manager_id2']."'"
                                     . " ,traffic_manager_id3 = '".$_REQUEST['traffic_manager_id3']."'"
                                    . " where id = $save_id;";
                            $this->dbi->query($sql);               
...
//                            die;
                    ...
                    if ($hdn_user_types == '107' && $_POST["chlDivisions"]) {
...
//            $this->dbi->query($sql);
                    ...
                    $userRoleId = $this->updateUserRoleByLevel($edit_id, $_REQUEST['selUserLevel']);

                    $userRoleId = $this->updateUserType($edit_id, $userType);
...
//            $formAttributeArr[] = array("chlDivisions", "Select Allied Division:", "id", $division_sql, $style, "u", "user_divisions");
        ...
        $this->editor_obj->button_attributes = array(
         ...
         if($_POST['idin']){
            ...
            if(!empty($_REQUEST['complianceDivision']) && $_REQUEST['hdnAction'] != '0'){            
               
                $complianceDivisions = implode(',', $_REQUEST['complianceDivision']);                
                $this->updateComplianceDivisions($_POST['idin'],$complianceDivisions);
               
            }
          
             $complianceDivision =  $this->getComplianceDivisions($_POST['idin']);
             if($complianceDivision){
             $complianceDivisionArray = explode(',',$complianceDivision);
             }else{
                 $complianceDivisionArray = array();
             }        
        }
        else{
          $complianceDivisionArray = array();  
...
//        prd($complianceDivisionArray);
//        die;
        $divisionSql = $this->getDivisionIds('all');

        //$selDivision = "<select name='complianceDivision[]'  id ='complianceDivision[]' multiple style=\"width: 500px;height: 100px !important;border-radius:2px !important\"><option value=\"\"> Select Division </option>";
        $selDivision = "<select name='complianceDivision[]'  id ='complianceDivision[]' style=\"width: 500px;border-radius:2px !important\"><option value=\"\"> Select Division </option>";
        foreach ($divisionSql as $key => $svalue) {
                 if(in_array($svalue['id'],$complianceDivisionArray)){
                     $selected = "selected";
                 }else{
                    $selected = "";
                }
        ...
        //$str .= '</br></br><div class="f1"> <b> Select Division </b>   '.$selDivision.'</div>';
    ...
    function copy_questions() {
      ...
      ";
        ...
        if ($_POST['idin']) {
            ...
            if(!empty($_REQUEST['complianceDivision'])){
                $complianceDivisions = implode(',',$_REQUEST['complianceDivision']);
                $this->updateComplianceEditor($complianceDivisions);
             }
        }
        $this->editor_obj->editor_template = '
...
//prd($_request);
        ...
        $listSql = "select user_notes.id as idin,
          user_notes.date as `Date`,
          user_notes.manager_name as `Manager Name`,
          user_notes.description as `Description`, 
          IF(user_notes.sign_on_off = 2,'Sign Off','Sign On') as `Last Status`,
          sign_on_times,
          IF(user_notes.sign_on_times IS NOT NULL && user_notes.sign_off_times IS NULL,concat('<a href=\"JavaScript:edit_record(',user_notes.id,')\"; >Sign off</a>'),user_notes.sign_off_times) as sign_off_times,
          CONCAT(users.name, ' ', users.surname) as `Edited By`,
    			'Edit' as `*`, 'Delete' as `!`
    			FROM user_notes
...
//      
        ...
        $this->editor_obj->table = "user_notes";
         ...
         $style_med = 'style="width: 300px;"';
         //   $style_med = 'style="width: 170px;"';
        $style_small = 'style="width: 120px;"';
        
        $edit_id = (isset($_POST['idin']) ? $_POST['idin'] : null);
        //$signOnOffArray = "select '1' as id, 'Sign On' item_name union all select '2', 'Sign Off';";
        if($edit_id){      
           //$signOnOffArray = "select '1' as id, 'Sign On' item_name union all select '2', 'Sign Off';";
            $signOnOffArray = "select '2' as id, 'Sign Off' item_name;";
            //$sonoff = '2';
        }else{
           $signOnOffArray = "select '1' as id, 'Sign On' item_name;"; 
            $sonoff = '1';
        }
       // $signOnOffArray = "select '1' as id, 'Sign On' item_name union all select '2', 'Sign Off';";
        ...
        $formAttributeArr = array();
            ...
            $formAttributeArr[] = array("calDate", "Date", "date","", $style_small . 'value="' . date('d-M-Y') . '"', "c", "","");
            
            $formAttributeArr[] = array("txaDescription", "Description", "description", "", 'style="width: 500px; height: 60px;"', "n", "","");            
            
            $formAttributeArr[] = array("txtManagerName", "Manager Name", "manager_name", "", $style_med, "c", $_SESSION['full_name'],"");
            
            $formAttributeArr[] = array("selSignOnOff", "Select Sign on Off", "sign_on_off",$signOnOffArray, $style_med, "c", "");
...
//            $formAttributeArr[] = array("calDob", "Date Of Birth", "calDob", "", $style, "", "");
            ...
            $resultV = $this->editor_obj->formAttributesVerticle($formAttributeArr);
            //print_r($resultV); die;
            $this->editor_obj->form_attributes = $resultV;
...
//        );
       ...
       if($_REQUEST["idin"]){
        $this->editor_obj->button_attributes = array(
            array("Save", "Reset"),
            array("cmdSave", "cmdReset"),
//            array("if(save()) this.form.submit()", "this.form.reset(); this.form.submit();"),
//            array("js_function_save", "js_function_delete", "js_function_edit")
 array("if(save()) this.form.submit()", "this.form.reset(); new_record(); this.form.submit();"),
array("js_function_save", "js_function_delete", "js_function_edit")
           ...
           $this->editor_obj->button_attributes = array(
            ...
            //array("if(add()) this.form.submit()", "if(save()) this.form.submit()", "this.form.reset(); this.form.submit();"),
            //array("js_function_add", "js_function_save", "js_function_delete", "js_function_edit")
        );
       }
        $this->editor_obj->form_template = '
    					
    					<div class="fl"><nobr>tcalDate</nobr><br />calDate</div>
                                        <div class="fl"><nobr>ttxtManagerName</nobr><br />txtManagerName</div>
    					<div class="fl"><nobr>ttxaDescription</nobr><br />txaDescription</div>
                                        <div class="fl"><nobr>tselSignOnOff</nobr><br />selSignOnOff</div>
              <div class="cl"></div>
              ' . $this->editor_obj->button_list();
        $this->editor_obj->editor_template = '
                <div class="fl">
    ...
    ';
       ...
       $this->list_obj->sql = "";
        ...
        if($form_action == "add_record" || $form_action == "save_record") {
                if($form_action == "add_record"){
                    $save_id = $this->editor_obj->last_insert_id;
                    //echo ""
                    $this->dbi->query("update user_notes set sign_on_times = '$time_date' where id = $save_id");
                    $this->dbi->query("select  sign_on_times from user_notes where id = $save_id");
                }else if($form_action == "save_record") {
                    $save_id = $this->editor_obj->idin;
                     
                    $this->dbi->query("update user_notes set sign_on_off = 2,sign_off_times = '$time_date' where id = $save_id");
                }
        }
    ...
    ';
         ...
         unset($_REQUEST['hdnAction']);
         
        
        $this->list_obj->sql = $listSql;
//        
//                      
        $str .= $this->editor_obj->draw_data_editor($this->list_obj,1);
    ...
    function Newsletters() {
        ...
        if ($preview_id || $send_id) {
            ...
            $sql = "select date_written, title, description,division_id from newsletters where id = $to_send_id;";
            $result = $this->dbi->query($sql);
            if ($myrow = $result->fetch_assoc()) {
        ...
        if ($preview_id) {
            // $str .= '<div class="fl">' . $this->division_nav($division_id, 'Edit/Newsletters', 0, 0,0,'&preview_id=5') . '</div>';
            //           if ($division_id == 'ALL'){
            //            $division_id = 0;
            //           }
            $str .= "<div class='cl'></div><br><br><h3>Newsletter Preview</h3><hr /><a href=\"" . $this->f3->get('main_folder') . "Edit/Newsletters?send_id=$to_send_id\">Click Here to Confirm and Send the Newsletter</a><hr />$mail_body";
             $str .= "<h3><a href='/Edit/Newsletters'>Back</a></h3>";
        } else if ($send_id) {
            ...
            while ($myrow2 = $result2->fetch_assoc()){
                ...
                while ($myrow3 = $result3->fetch_assoc()) {
                    ...
                    if($toEmail){
                        ...
                        $mail->Subject = "Allied Newsletter - $title";
                        $mail->Body = $send_body;
                
                        // Send the email
                        $mail->send();
                
                        $str .= "<h3>Message Sent to: $name &lt;$toEmail&gt;</h3>";
                    }
                }
            ...
            $this->list_obj->sql = "
            select newsletters.id as idin, date_written as `Date Written`,
            title as `Title`,
            ug.item_name as division,
            'Edit' as `*`, 'Delete' as `!`, CONCAT('[<a uk-tooltip=\"title: Preview and Send this Newsletter\" href=\"" . $this->f3->get('main_folder') . "Edit/Newsletters?preview_id=', newsletters.id, '\">Preview/Send</a>]') as `?`
            FROM newsletters
            LEFT JOIN companies ug ON ug.id = newsletters.division_id
            where 1 $filter_string
        ...
        ";
            ...
            $style = 'style="width: 220px;"';
            $style_large = 'style="width: 440px;"';
             $division_sql = "select 0 as id, '--- ALL ---' as item_name union select id, item_name from companies order by item_name";
              //$formAttributeArr[] = array("selDivisions", "Select Allied Division:", "id", $division_sql, "", "c", "");
            $this->editor_obj->form_attributes = array(
                array("calDateWritten", "txtTitle", "cmsDescription", "chlUserGroups",'selDivisions'),
                array("Date Written", "Title", "Description", "User Groups", "Select Allied Division:"),
                array("date_written", "title", "description", "id","division_id"),
                array("", "", "", $this->get_chl('user_group'),$division_sql),
            ...
            $this->editor_obj->form_template = '
                ...
                tcmsDescription<br />cmsDescription
                <br />
                 <div style="max-width: 700px;">tselDivisions<br />selDivisions</div>
    ...
    function Newsletterstest() {
        ...
        <head>
            ...
            <style>
                ...
                .container { width: 100%; max-width: 800px; margin: 0 auto; background-color: #fff; padding: 20px; }
                h1, h2, h3 { margin: 0; }
                p { margin: 0 0 15px; line-height: 1.6; color: #333; }
                ...
                .banner-text { position: absolute; top: 20px; left: 20px; background-color: orange; color: white; padding: 10px; font-size: 24px; font-weight: bold; }
                .date-tag { position: absolute; top: 20px; right: 20px; background-color: #0073b1; color: white; padding: 10px; font-size: 18px; }

                /* Section Styles */
                .section { padding: 20px 0; border-top: 1px solid #ddd; }
                .section-dark { background-color: #333; color: white; padding: 20px; }
                .button { display: inline-block; background-color: #000; color: white; padding: 10px 20px; text-decoration: none; font-weight: bold; border-radius: 5px; }

                /* Image Gallery */
                .gallery { display: flex; gap: 10px; justify-content: space-between; padding: 10px 0; }
                .gallery img { width: 30%; }

                /* Section Headers */
                .section-header { background-color: #d71c5d; color: white; padding: 10px; text-align: center; font-weight: bold; }
                .opportunities-header { background-color: #00b3e4; color: white; padding: 10px; text-align: center; font-weight: bold; }

                /* Footer Styles */
                .footer { background-color: #333; color: white; padding: 20px; text-align: center; }
                .footer .social-icons { margin: 10px 0; }
                .footer .social-icons a { display: inline-block; margin: 0 10px; color: white; text-decoration: none; }
                .footer .social-icons img { width: 24px; height: 24px; vertical-align: middle; }
            </style>
        </head>
        <body>
        ...
        <div class="container">
            ...
            <!-- Banner Section -->
            <div class="banner">
                <img src="https://picsum.photos/800/300" alt="Banner Image">
                <div class="banner-text">Indigenous Business Newsletter</div>
                <div class="date-tag">July 2024</div>
            </div>
            ...
            <div class="section">
                <p>Hi Mark,</p>
                <p>
                    We had an incredible time celebrating <span class="text-bold">NAIDOC Week 2024: Keep the Fire Burning</span> — amplifying Indigenous identity, voices, and cultures. From vibrant markets to insightful panels, concerts, and Corroborees in the park, each event was an unapologetic showcase of Blak excellence.
                </p>
                <p>
                    Excitement is now building for our flagship event <span class="text-bold">Connect</span>, heading to Meanjin (Brisbane) for the first time ever.
                </p>
                <p>
                    <span class="text-bold">Connect</span> is a powerful opportunity for the supplier diversity community to foster new opportunities and celebrate success. This year holds special significance as we mark 15 years of Supply Nation with the sector we exist to support.
                </p>
                <p>
                    Tickets to attend the <span class="text-bold">Tradeshow</span> are still available, but they are selling fast! Don’t miss the opportunity to network with motivated entrepreneurs and buyers — secure your ticket below.
                </p>
            </div>

            <!-- Call-to-Action Button -->
            <div class="section text-center">
                <a href="#" class="button">Secure your tickets</a>
            </div>

            <!-- NAIDOC Week Section -->
            <div class="section-header">NAIDOC Week out and about</div>
            <div class="section">
                <p>
                    Across the country, we gathered to celebrate NAIDOC Week and acknowledge the journey to equity that lies ahead.
                </p>
                <p>
                    We are proud to have been part of this celebration and remain committed to supporting the Indigenous business sector nationwide, year-round.
                </p>
                <div class="gallery">
                    <img src="https://picsum.photos/250/150" alt="Event Image 1">
                    <img src="https://picsum.photos/250/150" alt="Event Image 2">
                    <img src="https://picsum.photos/250/150" alt="Event Image 3">
                </div>
                <p class="text-center"><small>Left to right: Queensland Building and Construction Commission NAIDOC Panel, Sydney Town Hall Gadigal Markets, Musgrave Park Family Fun Day</small></p>
            </div>

            <!-- Opportunities Section -->
            <div class="opportunities-header">Opportunities</div>
            <div class="section">
                <h3>First Nations Director Scholarship Program</h3>
                <p>
                    Applications are open for the Australian Institute of Company Directors (AICD) First Nations Director Scholarship Program.
                </p>
                <p>
                    Funded by the National Indigenous Australians Agency (NIAA), the program is aimed at developing a cohort of Aboriginal and Torres Strait Islander leaders across Australia.
                </p>
                <p>
                    Fifty-six scholarships are available to directors and leaders reporting to the boards of both Indigenous and non-Indigenous organisations. Applications close <span class="text-bold">Sunday 25 August 2024, 11:59pm AWST</span>. Find out more <a href="#">here</a>.
                </p>

                <h3>Business grants up to $120,000 available</h3>
                <p>
                    Expressions of interest are open for Social Enterprise Development Initiative (SEDI) Grants.
                </p>
                <p>
                    Funded by the Australian Government’s Department of Social Services, the grants are aimed at helping social enterprises, including trading Indigenous-owned or controlled organisations with a defined social benefit, grow their business and scale their impact.
                </p>
                <p>
                    Grants are valued at up to $120,000 each. The program will continue until June 2026 with ongoing grant rounds. Find out more <a href="#">here</a>.
                </p>
            </div>

            <!-- Training Section -->
            <div class="section-header" style="background-color: orange;">Training for Indigenous businesses</div>
            <div class="section">
                <p>
                    Supply Nation offers a range of training and capability programs such as Drive to support you at every stage of your business journey.
                </p>
                <p>
                    We have now reached capacity at our August and September training sessions. Keep an eye out for updates on upcoming sessions.
                </p>
                <p>
                    Interested in talking to our corporate and government members on a supplier panel? Reach out to our team <a href="#">here</a>. This is an excellent opportunity to highlight your business and experiences with engaged procurement professionals.
                </p>
            </div>

            <!-- JumpStart Section -->
            <div class="section-header" style="background-color: orange;">Matching Indigenous businesses with volunteers</div>
            <div class="section">
                <p>
                    Proudly supported by Commonwealth Bank, JumpStart connects skilled representatives from our government and corporate members with the specific and real-time needs of Indigenous businesses.
                </p>
            </div>

            <!-- Member Opportunity Board Section -->
            <div class="opportunities-header">Member Opportunity Board</div>
            <div class="section">
                <p>
                    Supply Nation’s Member Opportunity Board is an online platform designed exclusively for Registered and Certified Suppliers to access the latest procurement opportunities from our corporate and government members.
                </p>
                <p>
                    Certified Suppliers will also receive email alerts with opportunities that match your business category. To access, sign in to your <a href="#">Supply Nation portal</a>.
                </p>
                <p><small>Member Opportunity Board opportunities available as of 25 July 2024: 44</small></p>
            </div>
            ...
            <div class="footer">
                ...
                <p>&copy; 2024 Indigenous Business Newsletter. All rights reserved.</p>
            </div>
