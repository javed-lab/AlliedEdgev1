...
// ************************************************************************************************************************************************************************
  ...
  function CurrentPeriodicals() {
    ...
    left join lookup_fields on lookup_fields.id = tasks.min_user_level_id
    left join companies on companies.id = tasks.division_id
    left join users on users.id = tasks.site_id
    where task_schedules.date = '$monday_select'
    ...
    ';
    $nav_str = $this->division_nav($division_id, 'Tasking/CurrentPeriodicals', 1, 0, 1);
    ...
    if($result = $this->dbi->query($sql)) {
      ...
      while($myrow = $result->fetch_assoc()) {
        if(!$division) {
          ...
          $str .= "<h3 class=\"fl\" style=\"padding-left: 20px;\">$site - Periodicals - Week Starting $week_starting</h3>";
          $str .= '<div class="fl" style="padding-left: 20px;">';
          for($x = 0; $x <= 4; $x++) {
            $str .= '<a uk-tooltip="'.($x ? ($x == 1 ? "Last Week" : "$x Weeks Ago") : "This Week").'" class="division_nav_item'.($week_select == $x ? " division_nav_selected" : "").'" href="'.$this->f3->get('main_folder').'Tasking/CurrentPeriodicals?week_select='.$x.'">'.($x ? $x : "This Week").'</a>';
          }
          
          $str .= ' &nbsp;<a uk-tooltip="View Current Month" class="division_nav_item" href="'.$this->f3->get('main_folder').'Tasking/ViewPeriodicals?month_filter='.$current_month.'">This Month</a>';
          $str .= ' &nbsp;<a uk-tooltip="View Whole Year" class="division_nav_item" href="'.$this->f3->get('main_folder').'Tasking/ViewPeriodicals">ALL</a>';
          
          $str .= '</div>';
          //$str .= '<div class="fr"><b>TEST DATE</b> '.date('l, d-M-Y', strtotime('2020-05-20')) . '</div>';
          $str .= '<div class="cl"></div>Click a Pending Task (in Orange) to Mark as Completed';
        ...
        if(($task_status == 'pending' || $task_status == 'late') && $is_admin && strtotime($test_date) <= strtotime($this_monday)) {
...
// TEST CODE
    $str .= $this->styles;
    $str .= '<div class="cl"></div><table class="grid"><tr><th>&nbsp;</th>';

      for($x = 1; $x <= 12; $x++) {
      $bg = ($x % 2 ? "w" : "g");
      $str .= '<th class="' . $cls . ';">' . ($month_filter ? '' : '<a href="'.$this->f3->get('main_folder').'Tasking/ViewPeriodicals?month_filter='.$x.'">') . date("M", strtotime("$current_year-$x")) . '</a></th>';
     }
    $str .= "</tr>";
    ...
    for($d = 0; $d < 7; $d++) {
      ...
      $str .= "<tr><td>{$days[$d]}</td><td colspan=\"12\">";
      for($x = 0; $x < 52; $x++) {
        $t = $first_monday_year + ($x * 7) + $d;
        $test_date = date('Y-m-d', strtotime("January 1st $current_year + $t days"));
        $day = date('d', strtotime($test_date));
        $cls = " task_complete pt";
        $str .= '<div id="' . "$task_id-$x" . '"'. $xtra . ' class="week_cell '. (!$x ? "left_cell" : "right_cell") . $cls . '">'.$day.'</div>';
      }
      $str .= "</td></tr>";
    }
    $str .= "</table>";
    ...
    if($result = $this->dbi->query($sql)) {
      ...
      while($myrow = $result->fetch_assoc()) {
        if(!$division) {
          ...
          $site = $myrow['site'];
          if(!$generate_schedule) {
            $str .= "<div class=\"fl\">" . $this->division_nav($division_id, 'Tasking/ViewPeriodicals', 1, 0, 1) . "</div>";

            $str .= '<h3 class="fl"> &nbsp;' . "$site - Periodicals</h3>";
            $str .= '<div class="fl" style="padding-left: 20px;">';
            $str .= '<a uk-tooltip="View Whole Year" class="division_nav_item'.(!$week_filter ? " division_nav_selected" : "").'" href="'.$this->f3->get('main_folder').'Tasking/ViewPeriodicals">ALL</a>';
            for($x = 1; $x <= 4; $x++) {
              $str .= '<a uk-tooltip="View Week '.$x.($current_week_of_month == $x ? "<br />(This Week)" : "").'" class="division_nav_item'.($week_filter == $x ? " division_nav_selected" : "").'" href="'.$this->f3->get('main_folder').'Tasking/ViewPeriodicals?week_filter='.$x.'">'.$x.($current_week_of_month == $x ? "*" : "").'</a>';
            }
            //$str .= " &nbsp;" . $this->week_nav;

            $str .= ' &nbsp;<a uk-tooltip="View This Week" class="division_nav_item" href="'.$this->f3->get('main_folder').'Tasking/CurrentPeriodicals">This Week</a>';
            if($month_filter != $current_month) $str .= ' &nbsp;<a uk-tooltip="View Current Month" class="division_nav_item" href="'.$this->f3->get('main_folder').'Tasking/ViewPeriodicals?month_filter='.$current_month.'">This Month</a>';

            $str .= '</div>';
            $str .= '<div class="fr"><b>TEST DATE</b> '.date('l, d-M-Y', strtotime('2020-05-20')).' &nbsp;&nbsp;<a class="list_a" href="'.$this->f3->get('main_folder').'Tasking/ViewPeriodicals?generate_schedule=1">Generate Schedule</a></div>';
            $str .= '<div class="cl"></div><table class="grid">';
            ...
            for($x = $startx; $x <= $finishx; $x++) {
              $cls = ($x % 2 ? "w" : "g");
              $str .= '<th colspan="4" class="' . $cls . ';">' . ($month_filter ? '' : '<a href="'.$this->f3->get('main_folder').'Tasking/ViewPeriodicals?month_filter='.$x.'">') . date("M", strtotime("$current_year-$x")) . '</a></th>';
             }
            $str .= "</tr>";
          }
        }
        ...
        $interval = ($frequency == "Monthly" ? 1 : ($frequency == "Quarterly" ? 3 : 6));
        if($generate_schedule) {
          if($frequency == 'Weekly') {
          ...
          if($frequency == 'Weekly') {
            ...
            for($x = $startx; $x <= $finishx; $x++) {
              ...
              for($y = 1; $y <= 4; $y++) {
                ...
                if(($task_status == 'pending' || $task_status == 'late') && $is_admin && $y == $week_of_month && $is_selected && strtotime($test_date) <= strtotime($this_monday)) {
...
//return $this->ta($sql_test);
      if($sqls) {
        ...
        $str .= '<h3>Schedule Generated...</h3><a class="list_a" href="'.$this->f3->get('main_folder').'Tasking/ViewPeriodicals'.'">Back to Periodicals</a>';
      }
    }
    
    if(!$division) {
      $str .= '<h3 class="fl">Periodicals</h3>';
      $str .= '<div class="fr" style="padding-right: 20px;">' . $this->division_nav($division_id, 'Tasking/ViewPeriodicals', 1, 0, 1) . '</div><div class="cl"></div>';
    }
...
//          if(DATE_ADD(tasks.date
      //DATE_FORMAT(date_add(date_sub(curdate(),INTERVAL dayofmonth(curdate())-1 DAY), 
      //INTERVAL (7-weekday(date_sub(curdate(),INTERVAL dayofmonth(curdate())-1 DAY)))+(FLOOR((DAYOFMONTH(tasks.date) - (if(DAYOFWEEK(CONCAT(DATE_FORMAT(tasks.date, '%Y-%m-01'))) > 2, 10, 3) - DAYOFWEEK(CONCAT(DATE_FORMAT(tasks.date, '%Y-%m-01'))))) / 7) + 1) = 5, 4, FLOOR((DAYOFMONTH(tasks.date) - (if(DAYOFWEEK(CONCAT(DATE_FORMAT(tasks.date, '%Y-%m-01'))) > 2, 10, 3) - DAYOFWEEK(CONCAT(DATE_FORMAT(tasks.date, '%Y-%m-01'))))) / 7) - 2)) WEEK), '%d-%b-%Y')


    $str = $this->division_nav($division_id, 'Tasking/EditPeriodicals', 1, 0, 1);
    ...
    if(!$is_admin) {
      ...
      $this->editor_obj->custom_field = "site_id";
      $this->editor_obj->custom_value = $site_id;
    }
    ...
    $this->editor_obj->form_attributes = array(
      ...
      array("date", "min_user_level_id", "description"),
