...
class TrainingAlliedAssignController extends Controller {
...
//    } else {
    ...
    function training_user_index() {

        $template = new Template;
        $view = new View;
        if ($_REQUEST['training_id'] == 0) {
            $training_id = 0;
        } else {
            $training_id = $_REQUEST['training_id'];
        }
...
//        $course_id = $lessonDetail['course_id'];
        ...
        $dataDetailQuery = "SELECT tar.start_date,tar.end_date,tar.training_title training_title,ta_courses.`id` course_id, ta_courses.`title` course_title,division.`item_name` division_name";
...
//prd($dataDetail);
        ...
        $this->f3->set('training_id', $training_id);
        $this->f3->set('dataDetail', $dataDetail);
        $this->f3->set('page_title', 'Training User List');
        $this->f3->set('base_url', $this->f3->get('base_url') . "/AlliedTraining/AssignTraining/");
        echo $template->render('header.htm');
        echo $template->render('nav.htm');
        echo $view->render('trainings/assign/training_index.php');
        echo $template->render('footer.htm');
    }
    ...
    function getTrainingUserAjax() {
        ...
        $where_cond = " tatu.training_rule_id = '" . $training_id . "' ";
//         $where_cond .= " and ta_questions.status != 2 ";
//         if($_REQUEST['lession_id']){
//              $where_cond .= " and ta_questions.lesson_id = ".$_REQUEST['lession_id'];
//         }else{
...
//         
        ...
        $select_records = " SELECT usr.ID AS user_id,CONCAT(usr.name, ' ', usr.surname) user_name,
           tatu.id training_id,CONCAT(clt.name, ' ', clt.surname) client_name,tatu.start_date start_date,tatu.end_date end_date,tac.id course_id,tar.training_title training_title,tal.id lesson_id,tal.title lesson_title,tac.id course_id, tac.title course_title, tatu.is_result";
...
//                    $edit = $delete = '';
                ...
                $jsonArray['data'][] = array(
                    ...
                    'client_name' => $val['client_name'] ? $val['client_name'] : "All Client",
                    'lesson_name' => $val['lesson_title'],
                    'start_date' => $val['start_date'],
                    'end_date' => $val['end_date'],
                        //'action' => "",
                );
