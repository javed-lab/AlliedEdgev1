...
class TrainingAlliedController extends Controller {
    ...
    function __construct($f3) {
     ...
     /*
      All Training user Chart List show
     */
...
//       $this->f3->set('Job_List', $job_list);
    ...
    /*
      user course Training status list
     * input user id and course id
     * return training list  for course 
     */
...
//        $this->f3->set('Job_List', $job_list);
         ...
         $this->f3->set('course_id', $courseId);
          $this->f3->set('user_id', $userId);
        $this->f3->set('page_title', 'User Training ');
    ...
    function getChartDetailAjax() {
        ...
        $where_cond .= " usr.id = '" . $_REQUEST['srch_user_id'] . "'";
        
          $where_cond .= " and tac.id = '" . $_REQUEST['srch_course_id'] . "'";
...
//        $this->f3->set('job_clients', $job_clients);
//        $this->f3->set('Job_List', $job_list);
        $this->f3->set('page_title', 'User Training Status');
        $this->f3->set('base_url', $this->f3->get('base_url') . "/AlliedMyTraining");

        echo $template->render('header.htm');
        echo $template->render('nav.htm');
        echo $view->render('trainings/training/mytrainingpending.php');
        echo $template->render('footer.htm');
    }
    ...
    function view() {
        ...
        if (!empty($_REQUEST)) {
            ...
            $select_records = " SELECT usr.ID AS userId,CONCAT(usr.name, ' ', usr.surname) employeeName,"
                    . "tatu.id trainingId,tatu.start_date startDate,tatu.end_date endDate,tatu.is_result,"
                    . " tar.training_title trainingTitle,"
...
//die;
            ...
            echo $template->render('header.htm');
            echo $template->render('nav.htm');
            echo $view->render('trainings/training/training_detail.php');
            echo $template->render('footer.htm');
            die;
        }
    }
...
//        die;
        ...
        if (!empty($_REQUEST['question'])) {
            ...
            foreach($questionNewArray as $key => $question) {
                ...
                $insertRec .= $commaAdd . '("' . $userTrainingId . '","' .$key.'","' . $question['question_txt'] . '","' . $question['selected_option_text'] . '","' . $question['selected_option'] . '","' . $question['answer_text_correct'] . '","' . $question['answer_correct'] . '","' . $answer_point . '")';
            }

//             echo $insertRec;
//            prd($questions);

            $deleteQuery = " delete from training_allied_user_training_statics  where training_id = '" . $userTrainingId . "'";
            $this->db->exec($deleteQuery);

            $insertQuery = "INSERT IGNORE INTO training_allied_user_training_statics 
                    (training_id,question_id,question, user_answer,user_answer_option,correct_answer,correct_answer_option,answer_point) 
                    VALUES 
                    $insertRec";
...
//            die;
        ...
        $where_cond = " tatu.id = $userTrainingId and tatu.user_id = '" . $_SESSION['user_id'] . "'";

        $select_records = " SELECT talq.*,tal.title,tar.training_title,tauts.user_answer_option";
//
        $afterFrom = " FROM training_allied_lesson_questions talq
                       LEFT JOIN training_allied_lessons tal ON tal.id = talq.lesson_id 
                       LEFT JOIN training_allied_courses tac ON tac.id = tal.course_id
                    INNER JOIN training_allied_rules tar ON tar.course_id = tac.id 
                  INNER JOIN training_allied_training_user tatu ON tatu.training_rule_id = tar.id
                  Left join training_allied_user_training_statics tauts on tauts.question_id = talq.id
                  "
                . " WHERE $where_cond";
        ...
        $this->f3->set('page_title', 'Training Paper');
        $this->f3->set('base_url', $this->f3->get('base_url'));

        echo $template->render('header.htm');
        echo $template->render('nav.htm');
        echo $view->render('trainings/training/training_paper.php');
        echo $template->render('footer.htm');
        die;
    }
