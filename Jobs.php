...
class Jobs extends Controller {
    ...
    function fetchJobsAjax() {
        ...
        $columnData = array(
            ...
            'jobs.job_title',
            'jobs.job_ref_no',
            'jobs.job_sub_type_id',
            'jobs.job_category_id',
            'jobs.job_client_id',
            'jobs.job_location_id',
            'jobs.job_supplier_id',
            'jobs.job_status',
            'jobs.created_at',
            'jobs.quote_end_date',
            'jobs.job_start_date',
...
//        }
        ...
        $select_records = "SELECT jobs.id, jobs.job_title, jobs.job_ref_no, jobs.job_status,jobs.job_start_date,jobs.job_end_date, jobs.created_at,jobs.quote_start_date,jobs.quote_end_date,jobs_sub_type.title as job_sub_type_title, jobs_category.category_name as job_category_name, CONCAT(client.name, ' ', client.surname) as client_name, CONCAT(location.name, ' ', location.surname) as location_name, CONCAT(supplier.name, ' ', supplier.surname) as supplier_name";
        $select_num_rows = "SELECT count(jobs.id) as total_records";
        $total_records = $this->db->exec("$select_num_rows FROM jobs LEFT JOIN jobs_sub_type ON jobs.job_sub_type_id=jobs_sub_type.id  LEFT JOIN jobs_category ON jobs.job_category_id=jobs_category.id  LEFT JOIN users as client ON jobs.job_client_id=client.id  LEFT JOIN users as location ON jobs.job_location_id=location.id LEFT JOIN users as supplier ON jobs.job_supplier_id=supplier.id WHERE $where_cond");
...
//        die;
        ...
        if ($total_data > 0) {
            $job_list = $this->db->exec("$select_records FROM jobs LEFT JOIN jobs_sub_type ON jobs.job_sub_type_id=jobs_sub_type.id  LEFT JOIN jobs_category ON jobs.job_category_id=jobs_category.id  LEFT JOIN users as client ON jobs.job_client_id=client.id  LEFT JOIN users as location ON jobs.job_location_id=location.id LEFT JOIN users as supplier ON jobs.job_supplier_id=supplier.id WHERE $where_cond  ORDER BY $order_by $sort_order LIMIT $length OFFSET $start");
        }
        ...
        if (!empty($job_list)) {
            ...
            foreach ($job_list as $key => $val) {
                ...
                if ($val['job_status'] == '4' || $val['job_status'] == '5')
                    $allocation = '';
                $jsonArray['data'][] = array(
                    'sr_no' => $start + $key + 1,
                    'job_title' => $val['job_title'],
                    'job_ref_no' => $val['job_ref_no'],
                    'job_sub_type_id' => $val['job_sub_type_title'],
                    'job_category_id' => $val['job_category_name'],
                    'job_client_id' => $val['client_name'],
                    'job_location_id' => $val['location_name'],
                    'job_status' => $jobStatusArr[$val['job_status']],
                    'job_supplier_id' => $val['supplier_name'] ? $val['supplier_name'] : "--",
                    'quote_end_date' => $val['quote_end_date'] ? date('d-m-y H:i:s', strtotime($val['quote_end_date'])) : "",
                    'job_start_date' => $val['job_start_date'] ? date('d-m-y H:i:s', strtotime($val['job_start_date'])) : "",
                    'job_end_date' => $val['job_end_date'] ? date('d-m-y H:i:s', strtotime($val['job_end_date'])) : "",
...
//         die;
    ...
    function fetchJobClientList() {
        ...
        if (!empty($_POST)) {
            $job_client_id = $this->f3->get('POST.job_client_id');
            $str .= '<option value="">Select job location</option>';
            if (!empty($job_client_id)) {
...
ORDER BY rt.start_time_date desc
    ...
    // Prepare CSV data
    $csvData = "Date,Location,Employee,Client,Assigned Hours,Start Time,End Time,Completed Hours\n";

    foreach ($roster_list as $row) {
        $csvData .= "{$row['DATE']},{$row['Location']},{$row['Employee']},{$row['Client']},{$row['assigned_hours']},{$row['start_time_date']},{$row['finish_time_date']},{$row['completed_hours']}\n";
    }

    // Set headers for CSV file download
    header("Content-type: text/csv");
    header("Content-Disposition: attachment; filename=roster_data.csv");
