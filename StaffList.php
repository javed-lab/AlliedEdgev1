...
class StaffList extends Controller {
  ...
  function Show() {
    ...
    // Check if '/' exists in the search string
    if (strpos($_REQUEST['search'], '/') !== false) {
        // If it exists, explode the string by '/'
        $get_search = explode('/', $_REQUEST['search'])[0]; // Assign the first part of the exploded array to $search
    }
    

    // $search = (trim($_REQUEST['search']) ? trim($_REQUEST['search']) : "");
    $search = (trim($get_search) ? trim($get_search) : "");
    ...
    if ($_SESSION['u_level'] >= 500 && $_SESSION['u_level'] >= 500) {
    if($search) {
       $search = "
    ...
    if($search) {
      $search = "
      WHERE (users.client_id LIKE 'L%'
      AND (users.client_id LIKE '%$clientIdSearch%'
   ...
   // If no search term is provided, show all records with client_id starting with 'L'
   $search = " WHERE users.client_id LIKE 'L%' " . ($_SESSION['user_id'] != 1 ? " AND users.id != 1 " : "");
}
  }
    $str .= '
    ...
    if($gid || $gid2 || $search || $userType || $userStatus) {
      $view_details->title = ($gid ? "Members of Group: $group_name" . ($gid2 ? " / " . $group_name2 : "") : "Search Results");      
...
CONCAT('<a target=\"_blank\" href=\"ShowImage?re_encrypt=1&no_crypt=1&i=induction_user_image/',users.id, '/user_',users.id,'.png\"><img onError=\"this.style.display=\'none\';\" style=\"max-height: 70px;\" src=\"Image?no_crypt=1&i=induction_user_image/', users.id, '/user_',users.id,'.png\"></a>', 
      ...
      if(users.commencement_date = '0000-00-00', '', CONCAT('<br/>', DATE_FORMAT(users.commencement_date, '%d-%b-%Y')))) as `Photo/Start Date`,
        ...
        $status_xtra $xtra CONCAT(users.name, if(users.preferred_name != '', CONCAT(' (', users.preferred_name, ')'), ''), ' ', users.surname) as `Full Name`, users.employee_id,
      ...
      if($homeSrch){
          ...
          while($myrow = $result->fetch_assoc()){
                ...
                $searchResult[$rkey]['employee_id'] = $myrow['employee_id'];
                $rkey++;
          }
